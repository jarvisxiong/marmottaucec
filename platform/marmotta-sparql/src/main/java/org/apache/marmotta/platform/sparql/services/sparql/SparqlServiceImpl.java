...
public class SparqlServiceImpl implements SparqlService {
    ...
    /**
     * @deprecated beginning with Sesame 2.8, use
     * {@link RDFFormat#getStandardURI()} or
     * {@link QueryResultFormat#etStandardURI()}
     */
    @Deprecated
        ...
        Future<Boolean> future = executorService.submit(new Callable<Boolean>() {
            ...
            public Boolean call() throws Exception {
                ...
                try {
                    ...
                    try {
                        connection.begin();
                        //extract GeoSPARQL Function to send SPARQL only to SESAME Sparql Validator

                        Query sparqlQuery = connection.prepareQuery(language, geoQueryValidator(query), configurationService.getBaseUri());
    ...
    /**
     * Evaluate a SPARQL query on the LMF TripleStore. Returns the results as a
     * list of result maps, each element a KiWiNode.
     * <p/>
    ...
    /**
     * Execute a SPARQL update on the LMF TripleStore. Throws a KiWiException in
     * case the update execution fails.
     * <p/>
     * see http://www.w3.org/TR/sparql11-update/
     *
     * @param queryLanguage
     * @param query a string representing the update query in SPARQL Update 1.1
     * syntax
     * @throws Exception
     */
    @Override
    ...
    private String geoQueryValidator(String query) {
        if (!query.contains("Intersect")) {
            System.out.println("QUERY: " + query);
            return query;
        } else {
            int indexInicioFilter = query.toUpperCase().indexOf("FILTER");

            //      System.out.println("FILTER FUNCTION: "  + query.substring(0, indexInicioFilter) + " " + query.substring(query.indexOf("}"), query.length())); 
            return query.substring(0, indexInicioFilter) + " " + query.substring(query.indexOf("}"), query.length());
        }
    }
